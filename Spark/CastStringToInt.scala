...
object CastStringToInt extends App {
  ...
  spark.sql("SELECT INT(salary),BOOLEAN(isGraduated),gender from CastExample").show()
  spark.sql("SELECT cast(salary as int) salary, BOOLEAN(isGraduated),gender from CastExample").show()
}